...
public enum SnapGeometry {
    ...
    /// Compute an initial position for a new item, avoiding existing items.
    ///
    /// Tries each enabled anchor in order, then falls back to cascade
    /// offsets from the first anchor.
    public static func initialPosition(
        from anchors: SnapAnchor,
        in canvasSize: CGSize,
        contentSize: CGSize,
        insets: SnapInsets,
        avoiding existing: [(position: CGPoint, size: CGSize)]
    ) -> CGPoint {
        let candidates = anchorPositions(
            for: anchors,
            in: canvasSize,
        ...
        for (_, position) in candidates {
            if !overlapsAny(position, size: contentSize, existing: existing) {
                return position
            }
        }

        // Try center if not already an anchor
        if !anchors.contains(.center) {
            if !overlapsAny(center, size: contentSize, existing: existing) {
                return center
            }
        }

        // Cascade from the first anchor position
        let base = candidates.first?.position ?? center
        return cascaded(
            from: base,
            in: canvasSize,
            contentSize: contentSize,
    ...
    /// Find the nearest enabled anchor that does not overlap any existing position.
    ///
    /// Falls back to cascade when all anchors are taken.
    public static func nearestAvailableAnchor(
        to point: CGPoint,
        from anchors: SnapAnchor,
        in canvasSize: CGSize,
        contentSize: CGSize,
        ...
        // Pick the nearest anchor without overlap
        if let free = sorted.first(where: { candidate in
            !overlapsAny(candidate.position, size: contentSize, existing: existing)
        }) {
            return free.position
        }

        // All anchors overlap — cascade from nearest
        let base = sorted.first?.position
    ...
    private static func overlapsAny(
        _ point: CGPoint,
        size: CGSize,
        existing: [(position: CGPoint, size: CGSize)]
    ) -> Bool {
        existing.contains { itemsOverlap(point, $0.position, sizeA: size, sizeB: $0.size) }
    }
    ...
    /// Place an item near `base` by trying 8 directional offsets,
    /// then falling back to a diagonal cascade.
    private static func cascaded(
        ...
        for (dx, dy) in offsets {
            ...
            let clampedPoint = clamped(candidate, in: canvasSize, contentSize: contentSize, insets: insets)
            if !overlapsAny(clampedPoint, size: contentSize, existing: existing) {
                return clampedPoint
            }
        }

        // Diagonal cascade in all four directions
        let step: CGFloat = 40
        let directions: [(CGFloat, CGFloat)] = [(1, 1), (-1, 1), (1, -1), (-1, -1)]
        for i in 1...maxCascadeAttempts {
            let offset = CGFloat(i)
            for (dx, dy) in directions {
                let candidate = CGPoint(x: base.x + dx * offset * step, y: base.y + dy * offset * step)
                let clampedPoint = clamped(candidate, in: canvasSize, contentSize: contentSize, insets: insets)
                if !overlapsAny(clampedPoint, size: contentSize, existing: existing) {
                    return clampedPoint
                }
